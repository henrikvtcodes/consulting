...
export const InviteModal = (props: {
  ...
  const { register, handleSubmit } = useForm<RequestInvites>();

  const { mutate } = useSWRConfig();

  const submitRequest = async (data: RequestInvites) => {
    console.log(data);
    try {
      await client.post(`invite/many?count=${data.count.toString()}`);
    } catch (e) {
      const error = e as HTTPError;

      toast.error("An error occurred while trying to create new invites");
    }
    mutate("invite/all");
    props.setOpen(false);
  };
